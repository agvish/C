...
void deleteVal(NODEPTR *plist,int key)												/*function definition to delete a node(s) with specified value*/
{
	...
	else
	{
		...
		while(curr != NULL)
		{
			if(curr->info == key && curr == *plist)									/*check for if the key matches with value and the node is the starting node of list*/
			{
			...
			if(curr->info == key && curr != *plist)									/*check for if the key matches with value and the node is not the starting node of list*/
			{
				count++;															/*Incrementing the counter*/
				delNode = curr;														/*holds the node to be deleted*/
