	...
	do{
		...
		switch(ch)
		{
			...
			case 1: printf("\nEnter a NODE value : ");
					...
					break;
			case 2: printf("\nEnter a NODE value to be removed : ");
					scanf("%d",&x);
...
{
	int found = 0, flag = 0;
	NODEPTR delNode, currNode, parent, insucc, pinsucc, temp;
	...
	do
	{
		if(currNode->info == val)							/*check if the node value matches*/
		{
			...
			{
				if(parent == NULL)						/*check if there is only one node in the tree*/
				{
					*proot = NULL;
					break;
				}
			...
			if(currNode->left == NULL || currNode->right == NULL)			/*check for node having only one child*/
			{
				...
				{
					if(parent == NULL)					/*check if there is only root with one right child in the tree*/
					{
				...
				{
					if(parent == NULL)					/*check if there is only root with one left child in the tree*/
					{
						*proot = currNode->left;
						break;
					}
			...
			else
			{
				...
				if(currNode->left != insucc)					/*check whether the left child of deleted node is not the successor*/
				{	
					temp = insucc;						/*Holds the successor node*/
					while(temp->left != NULL)				/*finds the leftmost child of successor node*/
						temp = temp->left;
					temp->left = currNode->left;				/*the leftmost child of successor node will hold the left subtree of deleted node*/
				}
				if(currNode->left == insucc)					/*check whether the left child of deleted node is the only successor*/
					insucc->left == currNode->left->left;  
				if(pinsucc != NULL)						/*check if there is a parent of successor node*/
					pinsucc->right = NULL;					/*parent node of successor has now no child*/
				break;
			}
		}
